...
const MusicPlayer: React.FC = () => {
  ...
  return (
    <Card className="fixed bottom-0 left-0 right-0 glass-intense border-t border-white/20 backdrop-blur-intense z-50 shadow-2xl mobile-player mobile-safe-bottom">
      {/* Animated Background */}
      ...
      <div className="relative z-10 p-3 sm:p-6">
        <div className="flex flex-col space-y-4 max-w-7xl mx-auto">
          ...
          <div className="md:hidden">
            {/* Currently Playing - Mobile */}
            <div className="flex items-center space-x-3 mb-4">
              <div className="relative w-12 h-12 rounded-lg overflow-hidden shadow-xl group flex-shrink-0">
              ...
              <div className="min-w-0 flex-1">
                <p className="text-white font-bold truncate mobile-text-base text-shadow">{currentSong.title}</p>
                <p className="text-gray-300 ultra-mobile-text-xs truncate font-medium">{currentSong.artist}</p>
              </div>
              ...
              {user && (
                <Button 
                  variant="ghost" 
                  size="sm" 
                  className={`mobile-player-control flex-shrink-0 transition-all duration-300 ${
            ...
            <div className="flex flex-col items-center space-y-3">
              <div className="flex items-center space-x-6">
                <Button 
                  variant="ghost" 
                  size="sm" 
                  className="mobile-player-control text-gray-400 hover:text-white"
                ...
                <Button
                  ...
                  className="mobile-player-control-primary shadow-2xl relative overflow-hidden group"
                >
              ...
              <div className="flex items-center space-x-3 w-full">
                <span className="ultra-mobile-text-xs text-gray-400 font-semibold w-10 text-right bg-white/5 px-2 py-1 rounded-lg">
                  {formatTime(progress)}
                </span>
                <div className="flex-1 relative">
                  <Slider
                    ...
                    max={duration}
                    step={1}
                    className="w-full mobile-player-progress"
                  />
                  {/* Progress Glow Effect */}
                  <div 
                    className="absolute top-1/2 left-0 h-1.5 bg-gradient-to-r from-purple-400 to-pink-400 rounded-full transform -translate-y-1/2 pointer-events-none blur-sm opacity-50"
                    style={{ width: `${(progress / duration) * 100}%` }}
                  ></div>
                </div>
                <span className="ultra-mobile-text-xs text-gray-400 font-semibold w-10 bg-white/5 px-2 py-1 rounded-lg">
                  {formatTime(duration)}
                </span>
              </div>
            </div>
